...
### How has COVID impacted catch and effort in the Pacific Canada groundfish fishery?

B. Connors (analysis) and L. Lacko (data)

`r Sys.Date()`

This document breifly describes analyses to quantify COVID-19 associated change in landings in the integrated groundfish fishery in Pacific Canada. 
...
agg_landed_spp <- catch_effort %>%
  group_by(year, month) %>%
  dplyr::summarise(landings = sum(landed_kg), n = n())%>%
  as.data.frame()
...
Landings clearly vary over the course of the year with max landings typically occuring over the summer. There is no uber obvious decline in landings in the spring/summer of 2020 coincident with COVID (note that landings in July should be interpreted with caution due to potentially incomplete reporting). But landings are dominated by Hake, so let's look at a few of the large landings species individually.
